...
var _preview_player: Node3D  # 繝悶Ο繝・け繝槭Φ縺ｮ繝ｫ繝ｼ繝・var _preview_emote_node: Node3D  # 迴ｾ蝨ｨ縺ｮFBX emote rig
...
func _process(dt: float) -> void:
	_preview_time += dt
	# WASD 繧ｫ繝｡繝ｩ遘ｻ蜍・	var move := Vector2.ZERO
...
func _build_ui() -> void:
	...
	preview_title.text = "式 繝繝ｳ繧ｹ繝励Ξ繝薙Η繝ｼ"
	...
	# 繧ｨ繝｢繝ｼ繝郁ｪｬ譏・	_emote_desc_label = Label.new()
	...
	# 隱ｬ譏・	var desc := Label.new()
	desc.text = "蜷・せ繝ｭ繝・ヨ縺ｫ螂ｽ縺阪↑繝繝ｳ繧ｹ繧貞牡繧雁ｽ薙※縲・n繧ｲ繝ｼ繝荳ｭ縺ｫ繧ｭ繝ｼ繧呈款縺励※繧ｨ繝｢繝ｼ繝育匱蜍包ｼ・
	...
	# 繧ｨ繝｢繝ｼ繝医げ繝ｪ繝・ラ・・ixamo鬚ｨ繧ｫ繧ｿ繝ｭ繧ｰ・・	var grid_title := Label.new()
	grid_title.text = "剥 繧ｨ繝｢繝ｼ繝井ｸ隕ｧ"
...
func _build_3d_preview() -> void:
	...
	# 繧ｫ繝｡繝ｩ 窶・霆碁％繧ｫ繝｡繝ｩ・・update_orbit_camera 縺ｧ豈弱ヵ繝ｬ繝ｼ繝譖ｴ譁ｰ・・	_preview_camera = Camera3D.new()
	...
	# 繝ｩ繧､繝・	var key_light := DirectionalLight3D.new()
	...
	# 蠎・＞蠎奇ｼ医げ繝ｪ繝・ラ鬚ｨ・・	var floor_mesh := MeshInstance3D.new()
	...
	# 繧ｰ繝ｪ繝・ラ邱・	for ix in range(-10, 11):
...
func _preview_emote(emote_id: int) -> void:
	...
	_emote_desc_label.text = EmoteData.get_emote_desc(emote_id)
	...
	# FBX繝｡繝・す繝･繧帝撼陦ｨ遉ｺ・医・繝ｼ繝ｳ繝・・繧ｿ縺縺大盾辣ｧ・・	for child in node.find_children("*", "MeshInstance3D", true, false):
		child.hide()

	# Skeleton繧呈爾縺励√・繝ｼ繝ｳ繝槭ャ繝斐Φ繧ｰ繧剃ｽ懈・・・layer_controller.gd縺ｨ蜷後§豁｣遒ｺ縺ｪ蜷榊燕繝槭ャ繝√Φ繧ｰ・・	_preview_skeleton = null
	...
	# AnimationPlayer繧呈爾縺励※繝吶せ繝医い繝九Γ繝ｼ繧ｷ繝ｧ繝ｳ繧貞・逕・	for child in node.find_children("*", "AnimationPlayer", true, false):
...
func _on_grid_emote_selected(emote_id: int, card: Button) -> void:
	...
	# 譁ｰ縺励＞驕ｸ謚槭ｒ繝上う繝ｩ繧､繝・	_selected_grid_btn = card
...
func _update_all() -> void:
	...
	var p_col: Color
	if _editing_player == 1:
		_player_toggle_btn.text = "泛 P1 險ｭ螳壻ｸｭ (竊・P2縺ｫ蛻・崛)"
		p_col = Color(0.95, 0.55, 0.20)
	else:
		_player_toggle_btn.text = "鳩 P2 險ｭ螳壻ｸｭ (竊・P1縺ｫ蛻・崛)"
	...
	for i in range(3):
		...
		# 繧ｭ繝ｼ繝ｩ繝吶Ν繧呈峩譁ｰ・医せ繝ｭ繝・ヨ縺ｮ隕ｪHBox縺ｮ譛蛻昴・蟄舌′繧ｭ繝ｼ繝ｩ繝吶Ν・・		var slot_hbox := _slot_labels[i].get_parent()
