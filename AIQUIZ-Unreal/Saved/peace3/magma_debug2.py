...
OUT = r"C:/AIQUIZ/AIQUIZ-Unreal/Saved/peace3/result_magma_debug2.json"
SHOT = r"C:/AIQUIZ/AIQUIZ-Unreal/Saved/peace3/magma_debug2.png"
DBG = "/Game/AiQuiz/Materials/M_MagmaDebug2"
res = {"ok": False, "log": []}
L = res["log"].append
...
try:
    ...
    if eal.does_asset_exist(DBG):
        mat = eal.load_asset(DBG); MEL.delete_all_material_expressions(mat)
    if not mat:
        at = unreal.AssetToolsHelpers.get_asset_tools()
        mat = at.create_asset("M_MagmaDebug2", "/Game/AiQuiz/Materials", unreal.Material, unreal.MaterialFactoryNew())
    mat.set_editor_property("shading_model", unreal.MaterialShadingModel.MSM_UNLIT)

    uv = E(mat, unreal.MaterialExpressionTextureCoordinate, -1400, -100)
    tnode = E(mat, unreal.MaterialExpressionConstant, -1400, 80)
    tnode.set_editor_property("r", 3.0)  # FIXED time=3.0 to rule out huge-editor-Time precision loss
    ...
    heat = E(mat, unreal.MaterialExpressionComponentMask, -780, 80)
    heat.set_editor_property("r", False); heat.set_editor_property("g", False)
    heat.set_editor_property("b", False); heat.set_editor_property("a", True)
    MEL.connect_material_expressions(custom, "", heat, "")
    # emissive = heat as grayscale (unlit) so brightness == heat value
    MEL.connect_material_property(heat, "", unreal.MaterialProperty.MP_EMISSIVE_COLOR)
    ...
    s.set_editor_property("auto_exposure_max_brightness", 0.5)
    ppv.set_editor_property("settings", s)

    ues = unreal.get_editor_subsystem(unreal.UnrealEditorSubsystem)
    # overhead over open magma to see the heat field as cells
    ues.set_level_viewport_camera_info(unreal.Vector(400, 9000, 6000), unreal.Rotator(0, -70, 90))
...
print("DBG2", res.get("ok"), res.get("err", ""))
