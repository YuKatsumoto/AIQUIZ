...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintNodeCommands::HandleConnectBlueprintNodes(const TSharedPtr<FJsonObject>& Params)
{
    ...
    {
        // Ghost event nodes (auto-placed BeginPlay/Tick templates) are disabled
        // by default and would be skipped by the compiler; enable on connect.
        for (UEdGraphNode* Node : { SourceNode, TargetNode })
        {
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintNodeCommands::HandleAddBlueprintVariable(const TSharedPtr<FJsonObject>& Params)
{
    ...
    {
        // UE5: float member variables are PC_Real with a PC_Double subcategory.
        // Using the legacy PC_Float category creates an invalid property that
        // crashes the Kismet compiler and asset serialization.
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintNodeCommands::HandleSetNodePinDefault(const TSharedPtr<FJsonObject>& Params)
{
    ...
    {
        return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Missing 'pin_value' parameter"));
    }
    ...
    for (UEdGraphPin* CandidatePin : TargetNode->Pins)
    {
        if (CandidatePin->Direction == EGPD_Input && CandidatePin->PinName.ToString().Equals(PinName, ESearchCase::IgnoreCase))
        {
            Pin = CandidatePin;
            break;
        }
    ...
    ResultObj->SetStringField(TEXT("default_value"), Pin->GetDefaultAsString());
    return ResultObj;
}
