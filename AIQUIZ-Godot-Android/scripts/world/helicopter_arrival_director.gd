...
const GP_APPROACH_DURATION := 3.60
const GP_OCEAN_STOP := 1.20
const GP_LADDER_DEPLOY := 0.80
const GP_LADDER_EXIT := 0.90
const GP_SWING_DURATION := 1.60
const GP_SWING_CYCLES := 2.25
const GP_SWING_AMPLITUDE_DEG := 48.0
const GP_LAUNCH_DURATION := 0.15
const GP_FLIGHT_DURATION := 1.45
const GP_LAND_HOLD := 0.55
const GP_HOVER_ALTITUDE := 11.40
const GP_OCEAN_OUTSET := 4.50
const GP_SLIDE_OCEAN_OFFSET := 4.00
const GP_P2_DELAY := 0.25
const GP_CABIN_PELVIS_HEIGHT := 0.55
...
func _menu_departure_remaining() -> float:
	if _menu_flight_profile != null and is_instance_valid(_menu_flight_profile):
		return maxf(_menu_flight_profile.get_runtime_duration() - _phase_elapsed, 0.0)
...
func _cabin_ride_transform(info: Dictionary) -> Transform3D:
	var wait_transform := _cabin_wait_transform(info)
	var holder := info.get("holder") as Node3D
	if holder == null or not is_instance_valid(holder):
		return wait_transform
	var cabin_basis := holder.global_basis.orthonormalized()
	wait_transform.origin = wait_transform.origin + cabin_basis.y.normalized() * (
		GP_CABIN_PELVIS_HEIGHT - CABIN_WAIT_PELVIS_HEIGHT
	)
	return wait_transform
...
func _prepare_gameplay_flight_paths() -> void:
	...
	for info: Dictionary in _helicopters:
		...
		var start := hover_ocean + Vector3(player_side * OFFSCREEN_DISTANCE, OCEAN_SIDE_APPROACH_RISE, 0.0)
...
func _update_authored_menu_departure(delta: float) -> bool:
	...
	if (
		...
		# Hold the live rope, rotors, and last authored pose until the wipe
		# finishes covering the screen. Completing here left an empty preview
...
func _ocean_slide_landing(info: Dictionary) -> Vector3:
	var ground: Vector3 = info.get("ground", Vector3.ZERO)
	var player_side := float(info.get("player_side", -1.0))
	var max_x := StageConstants.FLOOR_HALF_WIDTH - BELT_EDGE_LANDING_INSET
	var landing_x := clampf(
		ground.x + player_side * GP_SLIDE_OCEAN_OFFSET,
		-max_x,
		max_x
	)
	return Vector3(landing_x, ground.y, ground.z)
...
func _launch_gameplay_jump(info: Dictionary) -> bool:
	...
	var ocean_ground := _ocean_slide_landing(info)
	var landing := ocean_ground + Vector3.UP * 0.9
	...
	info["slide_landing"] = ocean_ground
	var ladder := info.get("rope_ladder") as PhysicalRopeLadder
...
func _update_gameplay_helicopter(info: Dictionary, delta: float) -> bool:
	...
	match phase:
		"approach":
			...
			var eased := smoothstep(0.0, 1.0, approach_t)
			...
			_update_intro_cabin_ride_passenger(info, false)
			if approach_t >= 1.0:
				_set_gp_phase(info, "ocean_stop")
		"ocean_stop":
			_set_flight_transform(holder, hover_ocean + bob, direction, 0.0, 0.0)
			var hatch := clampf(elapsed / HATCH_OPEN_DURATION, 0.0, 1.0)
			_set_hatch_openness(info, hatch)
			_update_intro_cabin_ride_passenger(info, hatch >= 0.35)
...
func _set_hatch_openness(info: Dictionary, openness: float) -> void:
	...
	if cabin_light != null and is_instance_valid(cabin_light):
		cabin_light.light_energy = CABIN_LIGHT_ENERGY * eased
