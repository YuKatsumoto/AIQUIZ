...
func _build_ui() -> void:
	...
	_skip_row = KeyHintRow.new()
	_skip_row.name = "SkipRow"
	_skip_row.alignment = BoxContainer.ALIGNMENT_BEGIN
	_skip_row.add_theme_constant_override("separation", 8)
	_skip_row.visible = false
	text_box.add_child(_skip_row)
...
func _refresh_content(model: Dictionary, locked: bool, compact: bool) -> void:
	...
	if _skip_row.visible:
		_skip_row.reset()
		_skip_row.add_spec("Enter", ACCENT, KeycapChip.SizeClass.SMALL)
		_skip_row.add_text("スキップ", MUTED_COLOR, 13)
...
func _create_chip(key_text: String, caption: String, done: bool, accent: Color) -> Control:
	...
	if KeyTokens.is_badge(key_text):
		stack.add_child(_create_badge_chip(key_text, done, accent))
	else:
		var keys := KeyHintRow.new()
		keys.alignment = BoxContainer.ALIGNMENT_CENTER
		keys.add_theme_constant_override("separation", 4)
		keys.add_spec(key_text, accent, KeycapChip.SizeClass.SMALL, done)
...
func _create_badge_chip(key_text: String, done: bool, accent: Color) -> Control:
	var key_panel := PanelContainer.new()
	var display_key := ("✓ " + key_text) if done else key_text
	key_panel.custom_minimum_size = Vector2(
		clampf(float(display_key.length()) * 10.0 + 22.0, 52.0, 138.0), 28.0
	)
	var style := StyleBoxFlat.new()
