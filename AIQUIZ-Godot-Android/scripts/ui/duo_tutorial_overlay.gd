...
func _chip_rows(model: Dictionary) -> Array[Dictionary]:
	...
	for player_variant: Variant in model.get("players", []):
		...
		if _is_mobile_profile():
			tasks = _mobile_tasks(player_index, tasks)
		rows.append({
			"label": "CPU" if _is_mobile_profile() and player_index == 2 \
				else str(player.get("label", "P%d" % player_index)),
...
func _is_mobile_profile() -> bool:
	return bool(ProjectSettings.get_setting("aiquiz/mobile/enabled", false)) \
		or OS.has_feature("mobile") or OS.has_feature("android") or OS.has_feature("ios")


func _mobile_tasks(player_index: int, source: Array) -> Array:
	var converted: Array = []
	for task_variant: Variant in source:
		var task: Dictionary = (task_variant as Dictionary).duplicate(true)
		var task_id := str(task.get("id", ""))
		if player_index == 2:
			task["key"] = "CPU"
			task["caption"] = "%s（自動）" % str(task.get("caption", "操作"))
		else:
			match task_id:
				"left", "right", "forward", "back", "ocean", "aim":
					task["key"] = "左スティック"
				"jump":
					task["key"] = "ジャンプ"
				"emote":
