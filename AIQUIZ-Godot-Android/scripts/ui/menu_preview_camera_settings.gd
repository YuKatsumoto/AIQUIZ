...
## メインメニュー背景カメラの保存・読込（user://menu_preview_camera.json）
## 起動時のカメラは menu_wall_background_preview.gd の PREVIEW_CAM_* のみ使用。
## JSON は F9 チューナーでの編集バックアップ用（起動時には読み込まない）。

const SAVE_PATH := "user://menu_preview_camera.json"
...
static func to_gdscript_constants(settings: Dictionary) -> String:
	...
	return (
		"const PREVIEW_CAM_POS := Vector3(%s, %s, %s)\n"
		% [_fmt(pos.x), _fmt(pos.y), _fmt(pos.z)]
		+ "const PREVIEW_CAM_ROT_DEG := Vector3(%s, %s, %s)\n"
		% [_fmt(rot.x), _fmt(rot.y), _fmt(rot.z)]
		+ "const PREVIEW_CAM_FOV := %s\n" % _fmt(merged["fov"])
		+ "const PREVIEW_CAM_H_OFFSET := %s" % _fmt(merged["h_offset"])
	)
