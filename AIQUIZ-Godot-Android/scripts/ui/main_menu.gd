...
@onready var legacy_2d_btn: Button = get_node_or_null("%Legacy2dBtn") as Button
...
func _ready() -> void:
	...
	_style_all_buttons()
	_setup_legacy_2d_button()
	_apply_mobile_menu_layout()
...
func _apply_mobile_menu_layout() -> void:
	if not _is_mobile_profile():
		return
	# 16:9の論理座標は維持し、ノッチ付近と親指操作に余白を持たせる。
	menu_vbox.offset_left = 56.0
	menu_vbox.offset_right = 744.0
	for button: Button in _get_all_buttons(self):
		...
		button.add_theme_font_size_override(
			"font_size",
			maxi(button.get_theme_font_size("font_size"), 17)
		)
	for carousel_button: Button in [
		prev_grade_btn, next_grade_btn,
		prev_subject_btn, next_subject_btn,
		prev_diff_btn, next_diff_btn,
	]:
		carousel_button.custom_minimum_size.x = maxf(carousel_button.custom_minimum_size.x, 72.0)
	start_button.custom_minimum_size.y = maxf(start_button.custom_minimum_size.y, 64.0)
	if res_option != null and res_option.get_parent() is CanvasItem:
		(res_option.get_parent() as CanvasItem).visible = false
	if legacy_2d_btn != null:
		legacy_2d_btn.visible = false
...
func _begin_scene_change_after_helicopter(
	...
	if departure_started:
		...
		if (
			...
			push_warning("Menu helicopter departure timed out; covering while the sequence continues")
	if is_inside_tree():
...
func _setup_legacy_2d_button() -> void:
	if legacy_2d_btn == null:
		legacy_2d_btn = get_node_or_null("%Legacy2dBtn") as Button
	if legacy_2d_btn == null:
		return
	legacy_2d_btn.text = "旧版2D（オフライン）で遊ぶ"
	if not legacy_2d_btn.pressed.is_connected(_on_legacy_2d_pressed):
		legacy_2d_btn.pressed.connect(_on_legacy_2d_pressed)

func _on_legacy_2d_pressed() -> void:
	var script_path := _resolve_legacy_2d_script()
	if script_path.is_empty() or not FileAccess.file_exists(script_path):
		api_status_label.text = "[color=red]旧版 2D_pygame.py が見つかりません。[/color]\n想定場所: ../AIQUIZ-v1/AIQUIZ-v1/2D_pygame.py"
		return
	var python_cmd := _resolve_python_executable()
	var pid := OS.create_process(python_cmd, PackedStringArray([script_path]), true)
	if pid < 0:
		var bat_path := ProjectSettings.globalize_path("res://tools/play_legacy_2d_offline.bat")
		if FileAccess.file_exists(bat_path):
			OS.shell_open(bat_path)
			api_status_label.text = "[color=green]旧版2D（オフライン）を起動しました。[/color]"
		else:
			api_status_label.text = "[color=red]旧版2Dの起動に失敗しました。Python が PATH にあるか確認してください。[/color]"
	else:
		api_status_label.text = "[color=green]旧版2D（オフライン）を起動しました。[/color]"

func _resolve_legacy_2d_script() -> String:
	var godot_root := ProjectSettings.globalize_path("res://").rstrip("/\\")
	var sibling := godot_root.get_base_dir().path_join("AIQUIZ-v1").path_join("AIQUIZ-v1").path_join("2D_pygame.py")
	if FileAccess.file_exists(sibling):
		return sibling
	return ""

func _resolve_python_executable() -> String:
	var local_py := "C:/Users/kykat/AppData/Local/Programs/Python/Python310/python.exe"
	if FileAccess.file_exists(local_py):
		return local_py
	return "python"
