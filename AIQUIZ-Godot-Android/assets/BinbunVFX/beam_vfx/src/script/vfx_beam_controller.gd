...
var audio_mid : AudioStreamPlayer3D:
	get():
		var scalor := get_node_or_null("BeamPivot/BeamScalor") as Node3D
		if scalor == null:
			return null
		var result := scalor.find_child("AudioMid", false, false) as AudioStreamPlayer3D
		# Beam 01 in the purchased pack names this middle player AudioEnd.
		if result == null:
			result = scalor.find_child("AudioEnd", false, false) as AudioStreamPlayer3D
		if !Engine.is_editor_hint():
			audio_mid = result
		return result
...
func _set_open_amount() -> void:
	if not beam_scalor or not beam_pivot or not beam_particles:
		return
