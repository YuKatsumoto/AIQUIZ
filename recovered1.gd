...
func _show_waiting_start(dt: float) -> void:
	...
	if game_state.mode == Constants.MODE_TUTORIAL:
		pl_status.text = "3Dチュートリアル準備完了"
		start_prompt_label.text = "[ 任意のキーを押して練習スタート ]"
	else:
...
func _create_tutorial_panel() -> void:
	...
	var style := StyleBoxFlat.new()
	style.bg_color = Color(0.04, 0.06, 0.12, 0.86)
	style.border_color = Color(0.30, 0.48, 0.80, 0.65)
	style.set_border_width_all(1)
	...
	_tutorial_panel.offset_left = 120.0
	_tutorial_panel.offset_top = -150.0
	_tutorial_panel.offset_right = -120.0
	_tutorial_panel.offset_bottom = -44.0
	...
	_tutorial_title.add_theme_color_override("font_color", Color(1.0, 0.88, 0.25))
	...
	_tutorial_detail.add_theme_font_size_override("font_size", 17)
	_tutorial_detail.add_theme_color_override("font_color", Color(0.84, 0.88, 0.96))
	_tutorial_detail.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	box.add_child(_tutorial_detail)
...
func _update_tutorial_panel() -> void:
	...
	if game_state.mode != Constants.MODE_TUTORIAL:
		_tutorial_panel.visible = false
		return
	if game_state.game_state in [Constants.STATE_FLYOVER, Constants.STATE_GAME_OVER]:
		_tutorial_panel.visible = false
		return
	if game_state.game_state == Constants.STATE_CLEAR:
		_tutorial_panel.set_anchors_preset(Control.PRESET_TOP_WIDE)
		_tutorial_panel.offset_left = 160.0
		_tutorial_panel.offset_top = 88.0
		_tutorial_panel.offset_right = -160.0
		_tutorial_panel.offset_bottom = 176.0
	else:
		_tutorial_panel.set_anchors_preset(Control.PRESET_BOTTOM_WIDE)
		_tutorial_panel.offset_left = 120.0
		_tutorial_panel.offset_top = -150.0
		_tutorial_panel.offset_right = -120.0
		_tutorial_panel.offset_bottom = -44.0
	var title := game_state.tutorial_instruction_text()
	var detail := game_state.tutorial_detail_text()
	_tutorial_panel.visible = not title.is_empty()
	_tutorial_title.text = title
	_tutorial_detail.text = detail
...
func _update_score() -> void:
	if game_state.game_state in [Constants.STATE_PLAYING, Constants.STATE_CORRECT, Constants.STATE_GOAL_RACE]:
		...
		if game_state.num_players >= 2:
			...
			if game_state.mode == Constants.MODE_TUTORIAL:
				score_label.text = "練習: %d/%d" % [mini(game_state.current_index + 1, game_state.target_count), game_state.target_count]
			elif game_state.mode == Constants.MODE_TEN:
...
func _update_progress() -> void:
	if game_state.mode == Constants.MODE_TUTORIAL and \
			game_state.game_state in [Constants.STATE_PLAYING, Constants.STATE_CORRECT]:
		progress_bar.visible = true
		progress_bar.max_value = game_state.target_count
		progress_bar.value = game_state.current_index
	elif game_state.mode == Constants.MODE_TEN and \
			game_state.game_state in [Constants.STATE_PLAYING, Constants.STATE_CORRECT]:
		progress_bar.visible = true
